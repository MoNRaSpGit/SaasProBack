...
async function main() {
  ...
  const email = `distribuidora-${suffix}@saaspro.test`;
  const password = "demo12345";
  const fullName = `Distribuidora Demo ${suffix}`;
  const tenantName = `Distribuidora Tenant ${suffix}`;
  ...
  try {
    ...
    const statusResponse = await fetch(`${baseUrl}/distribuidora/status`, {
      headers: {
    ...
    if (!statusResponse.ok) {
      throw new Error(`Distribuidora status failed: ${JSON.stringify(statusPayload)}`);
    }

    const adminStatusResponse = await fetch(`${baseUrl}/distribuidora/admin/status`, {
      headers: {
        Authorization: `Bearer ${accessToken}`
      }
    });
    const adminStatusPayload = await readJson(adminStatusResponse);
    if (!adminStatusResponse.ok) {
      throw new Error(`Distribuidora admin status failed: ${JSON.stringify(adminStatusPayload)}`);
    }
      ...
      JSON.stringify(
        {
          ...
          shellStatus: statusPayload,
          adminShellStatus: adminStatusPayload
        },
        null,
        2
      )
