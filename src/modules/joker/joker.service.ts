...
export class JokerService {
  ...
  // la grafica usan el cierre diario congelado (saas_joker_cierres_diarios)
  // como fuente de verdad para cualquier dia comercial que ya lo tenga --
  // solo cae a calcular en vivo desde saas_joker_orders para dias que
  // todavia no se cerraron (tipicamente hoy, antes de que corra el cron
  // de las 5am). El "dia" aca es siempre el dia comercial (5am a 5am, ver
  // getStoreDateLabel), no el dia calendario.
  ...
  private async getOrdersLiveByDay(startIso: string, endIso: string): Promise<Map<string, number>> {
    ...
    for (const row of rows) {
      const clave = this.getStoreDateLabel(new Date(row.created_at as string));
  ...
  private async getCierresPorDia(primerDia: string, ultimoDia: string): Promise<Map<string, number>> {
    ...
    const porDia = new Map<string, number>();
    for (const row of rows) {
      porDia.set(this.toIsoDateOnly(row.fecha), Number(row.total) || 0);
    }
    return porDia;
  }
  ...
  private buildMonthRangeUtc(anio: number, mes: number): { startIso: string; endIso: string; daysInMonth: number } {
    ...
    const { startIso } = this.buildStoreDayRangeUtc(primerDia);
    const { endIso } = this.buildStoreDayRangeUtc(ultimoDia);
  ...
  async getMonthHistory(cantidad: number): Promise<{ items: JokerMonthHistoryItem[] }> {
    ...
    const { anio: anioActual, mes: mesActual } = this.anioMesActualStore();
  ...
  // Rango del dia comercial anterior (el que acaba de terminar) -- usado
  // por el cron de las 5am para cerrarlo.
  private buildPreviousStoreDayRangeUtc(): { startIso: string; endIso: string; fechaYMD: string } {
    const previousDayInstant = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const fechaYMD = this.getStoreDateLabel(previousDayInstant);
    const { startIso, endIso } = this.buildStoreDayRangeUtc(fechaYMD);
    return { startIso, endIso, fechaYMD };
  }

  private anioMesActualStore(): { anio: number; mes: number } {
    const [anio, mes] = this.getStoreDateLabel().split("-").map(Number);
    return { anio, mes };
  }
  ...
  async cerrarDiaAutomatico(): Promise<void> {
    const { startIso, endIso, fechaYMD } = this.buildPreviousStoreDayRangeUtc();
